...
describe('dashboard HTTP (database double, real auth guard)', () => {
  ...
  const db = { product: {count:vi.fn(),fields:{minimumStock:'minimumStock'}}, invoice: { aggregate: vi.fn() }, authSession: { findUnique: vi.fn() }, businessMember: { findUnique: vi.fn() } };
  beforeEach(async () => {
    vi.resetAllMocks();
  ...
  it('returns authorized business and truthful unavailable metrics without sensitive details', async () => {
    ...
    const { customers, suppliers, products, lowStock, todaySales, monthlySales, ...future } = result.body.metrics;
