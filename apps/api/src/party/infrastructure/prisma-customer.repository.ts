...
import { defined, editable, listWhere, ownerScope, partyData, partyListSelect, rethrowParty, required, requireOwner, serialize } from '../../parties/party.data.js';
import type { SafeUser } from '../../users/user.select.js';
import type { PartyQuery } from '../../parties/party.dto.js';
import type { CustomerCommand, CustomerDetail, CustomerListItem, CustomerProfile, PartyActor, PartyListQuery, PartyListResult } from '../domain/party-types.js';
...
export class PrismaCustomerRepository implements CustomerRepositoryPort {
  ...
  private data(dto: CustomerCommand) {
    const shipping = dto.shippingSameAsBilling ?? true;
    if (!shipping) validatePartyState(required(dto.shippingState, 'shippingState'), required(dto.shippingStateCode, 'shippingStateCode'));
    return {
      ...
      shippingAddressLine1: shipping ? null : required(dto.shippingAddressLine1, 'shippingAddressLine1'),
      shippingAddressLine2: shipping ? null : dto.shippingAddressLine2 || null,
      shippingCity: shipping ? null : required(dto.shippingCity, 'shippingCity'),
      shippingState: shipping ? null : required(dto.shippingState, 'shippingState'),
      shippingStateCode: shipping ? null : required(dto.shippingStateCode, 'shippingStateCode'),
      shippingPincode: shipping ? null : required(dto.shippingPincode, 'shippingPincode'),
      creditLimit: dto.creditLimit ? new Prisma.Decimal(dto.creditLimit) : null,
    };
  }

  async create(user: PartyActor, dto: CustomerCommand): Promise<CustomerProfile> {
  ...
  async list(user: PartyActor, query: PartyListQuery): Promise<PartyListResult<CustomerListItem>> {
    return this.db.$transaction(async (tx) => {
      const scope = await requireOwner(tx, user as SafeUser);
      const where: Prisma.CustomerWhereInput = { ...scope, ...listWhere(query as PartyQuery), ...(query.search ? { OR: ['customerCode', 'displayName', 'businessName', 'phone', 'email', 'gstin'].map((field) => ({ [field]: { contains: query.search, mode: 'insensitive' } })) } : {}) };
  ...
  private profile(row: Prisma.CustomerGetPayload<object>): CustomerProfile {
    ...
    return { ...serialize(profile), creditLimit: row.creditLimit?.toFixed(2) ?? null } as CustomerProfile;
  }

  async detail(user: PartyActor, id: string): Promise<CustomerDetail> {
    const row = await this.db.customer.findFirst({ where: { ...ownerScope(user as SafeUser), id } });
    if (!row) throw new NotFoundException('Customer not found');
    const [summary, paid] = await Promise.all([
      this.db.invoice.aggregate({ where: { ...ownerScope(user as SafeUser), customerId: id, status: 'FINALIZED' }, _sum: { grandTotal: true }, _count: { id: true } }),
      this.db.paymentAllocation.aggregate({ where: { businessId: user.currentBusinessId!, invoice: { customerId: id, status: 'FINALIZED' }, payment: { status: 'POSTED', type: 'CUSTOMER_RECEIPT' } }, _sum: { amount: true } }),
    ]);
  ...
  async update(user: PartyActor, id: string, dto: CustomerCommand): Promise<CustomerProfile> {
    try {
      return await this.db.$transaction(async (tx) => {
        const scope = await requireOwner(tx, user as SafeUser);
        const row = await tx.customer.findFirst({ where: { ...scope, id } });
        if (!row) throw new NotFoundException('Customer not found');
        const data = this.data({ ...editable(row), ...defined(dto) } as CustomerCommand);
        const result = await tx.customer.updateMany({ where: { ...scope, id }, data });
        if (result.count !== 1) throw new NotFoundException('Customer not found');
        return this.profile(await tx.customer.findFirstOrThrow({ where: { ...scope, id } }));
