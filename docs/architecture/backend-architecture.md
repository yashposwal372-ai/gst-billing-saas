...
## A09 first physical service stabilization: 2026-09-17

A09 stabilizes the A08 `apps/party-service` extraction without extracting another service. The production topology remains: Next.js frontend -> API Gateway foundation -> `apps/api` public monolith -> internal HTTP client -> `apps/party-service` -> shared PostgreSQL. The Gateway does not route directly to party-service, and the frontend remains on the existing public API surface.

A permanent workflow lives at `.github/workflows/ci.yml`. It uses `ubuntu-latest`, Node 24, npm 11 through repository `devEngines`, `npm ci`, read-only repository permissions and no deployment/cloud secrets. The deterministic job runs lint, architecture, gateway, party-service, OpenAPI, typecheck, build and relevant unit/e2e suites. A separate PostgreSQL 17 job uses synthetic service-container credentials, runs `npm run prisma:deploy --workspace api` against an empty database, validates Prisma and runs `npm run test:postgres --workspace party-service`.

The party-service real PostgreSQL tests prove the first physical service boundary with real HTTP, real `@gst/security-context` verification, real Prisma adapter and real database rows. They verify Customer and Supplier persistence, signed-business tenant isolation, tenant-scoped GSTIN uniqueness, exact Decimal opening-balance string serialization, deactivate/reactivate persistence, health/live and health/ready semantics, unavailable and timeout mapping from the monolith `HttpPartyServiceClient`, no hidden retry attempts, and Nest/Prisma shutdown ownership. These tests are opt-in locally using `RUN_PARTY_POSTGRES_TESTS=1` and `TEST_DATABASE_URL`.

Single-writer ownership is enforced by `npm run party-service:check`. Production Customer/Supplier writes are expected only in `apps/party-service` infrastructure. The monolith `PartyModule` must remain wired to `HttpPartyServiceClient`; legacy monolith Prisma repositories may remain as transitional source but are not production-wired. Transitional shared-database readers remain read-only inventory for invoices, sales and purchase documents, finance, business documents, POS, dashboard, GST reports and related projections.

A09 does not introduce semantic Prisma schema changes, migrations, a Party-owned database, Kafka, outbox/inbox, Redis Party cache, Gateway direct Party routing, frontend cutover, Phase 11 work or another service extraction. Minimal boundary logging records service, operation, outcome/status, requestId and correlationId only; it must not log cookies, JWTs, internal signatures, full internal context, GSTIN/PAN, bank data, financial payloads or passwords.
